...
export default function ProviderSupportPage() {
  ...
  async function sendReply(id: string) {
    ...
    await fetch(`/api/support/tickets/${id}/messages`, {
      ...
      body: JSON.stringify({ body: reply, authorRole: 'PROVIDER' }),
    })
    setReply('')
    setSaving(false)
    load()
  ...
  return (
    <div className="p-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold">Provider Support</h1>
          <p className="text-sm text-slate-500">Reply to active support requests from patients.</p>
        </div>
      </div>
              ...
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  {(ticket.orderNumber || ticket.orderId) && (
                    <div className="text-xs text-slate-500 mt-1">Order: {getOrderRef(ticket.orderNumber, ticket.orderId)}</div>
                  )}
                </div>
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  {(selected.orderNumber || selected.orderId) && (
                    <p className="mt-2 text-xs text-slate-600">Linked order: <span className="font-mono">{getOrderRef(selected.orderNumber, selected.orderId)}</span></p>
                  )}
                </div>
